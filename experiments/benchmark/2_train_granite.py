...
if __name__ == "__main__":
    ...
    background = subsample_data(x_train, args.granite.train_background_size, args.random_state)
    ...
    loss_fn = get_granite_loss_functions(decomposition, background, CAT_FEATURES_INDICES[args.dataset])

    # TODO can we handle depth=0???
    ...
    test_background = subsample_data(x_test, args.granite.test_background_size, args.random_state)
    ...
    for r in range(tree.n_regions):
        ...
        for metric in ["PDP_vs_ICE", "PDP_vs_Mplot"]:
            loss_fn = get_granite_loss_functions(regional_decomposition, regional_background, CAT_FEATURES_INDICES[args.dataset])

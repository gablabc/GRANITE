...
from utils import setup_data, get_train_test_split, load_model, subsample_data, CAT_FEATURES_INDICES
...
if __name__ == "__main__":
    ...
    background, y_background = subsample_data(x_train, y_train, args.granite.train_background_size, args.random_state)
    filename = os.path.join("models", args.dataset, f"{args.model_name}_{args.random_state}", f"decomposition_{args.granite.train_background_size}.joblib")
    if os.path.exists(filename):
        decomposition = load(filename)
    else:
        decomposition = get_functional_decomposition(background, model, args.model_name, features=features)
        dump(decomposition, filename)
    loss_fn = get_granite_loss_functions(decomposition, background, y_background, task, CAT_FEATURES_INDICES[args.dataset])

    tree = FDTree(max_depth=args.granite.max_depth, features=features, alpha=args.granite.alpha)
    ...
    test_background, y_background = subsample_data(x_test, y_test, args.granite.test_background_size, args.random_state)
    ...
    for r in range(tree.n_regions):
        ...
        regional_decomposition = get_functional_decomposition(regional_background, model, args.model_name, features=features)
        loss_fn = get_granite_loss_functions(regional_decomposition, regional_background, regional_y, task, CAT_FEATURES_INDICES[args.dataset])
        for metric in disagreements.keys():
            disagreements[metric] += weights[r]* loss_fn[metric](np.arange(N_r)) / N
    save_GRANITE_disagreements(disagreements, args.dataset, args.model_name, args.random_state, args.granite)
